...
const saveDraft = (url, data, method) => {
...
const renderDraft = (draftArr) => {
  ...
  draftArr.map((item) => {
    ...
    controls.classList = selector === 'p' ? 'paragraph-controls mb-3' : 'heading-controls mb-3';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  previewBtn.addEventListener('click', () => {
    ...
    previewBtn.textContent = previewBtn.textContent === 'Preview' ? 'Edit' : 'Preview';
...
const addMoreEventInfoClickHandler = () => {
  document.querySelector('.main-content').addEventListener('click', (event) => {
    ...
    if (targetClassList.contains('add') || targetClassList.contains('add-icon')) {
      const toggleIcon = add.querySelector('.bi');
    ...
    if (targetClassList.contains('save-paragraph')) {
      ...
      if (!saved) {
        alert('The size of the paragraph must be between 1 to 2000 characters.');
        return;
      }
    ...
    if (targetClassList.contains('save-btn')) {
      ...
      if (!editContent(container)) {
        alert('Sorry, we couldn\'t save your content. If this keeps happening, save your draft and reload the page');
        return;
      }
      editSaveToggleContent(type, event.target, container);
      return;
    ...
    if (targetClassList.contains('delete-btn')) {
      const del = confirm('Are you sure you want to delete this text? This action cannot be undone.');
      if (del) {
        const type = targetClassList.contains('h') ? 'h' : 'p';
        const container = event.target.closest(`.${type}-wrapper`);
    ...
    if (targetClassList.contains('remove-textarea')) {
      const container = event.target.closest('.paragraph-container');
      const removeContainer = confirm('The input box including its content will be removed. Are you sure about this?');
      if (removeContainer) {
        container.remove();
      }
    }
...
const editSaveToggleContent = (type, btn, container) => {
  ...
  if (content.textContent.length === 0 || content.textContent.length > maxLength) {
    alert(`The size of this text must be between 1 to ${maxLength} characters.`);
    return;
  }
  ...
  content.contentEditable = content.contentEditable === 'inherit' ?
    true : 'inherit';
}
